...
import { calculateTotalDays } from '../utils/totalDays';


const HeroSection = () => {
    const numbersOfPlaceByCountry= {}

    const {isTravelModal} = useOutletContext()
    const {trips} = useSelector((store) => store.trip)
    
    const lastAdventure = trips[trips.length-1]
    ...
    trips.forEach((trip) => {
        ...
        if(!numbersOfPlaceByCountry[country]){
            numbersOfPlaceByCountry[country] = 0
        }
        numbersOfPlaceByCountry[country] += 1
    })
    
    const totalPlace = Object.values(numbersOfPlaceByCountry).reduce((acc,curr) => {
        return acc + curr
    }, 0)
        ...
        <div className='relative h-screen'>
            ...
            <div className='relative grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-4 place-items-center  h-full z-10 p-20 md:p-6 '>
                <div className='flex flex-col gap-2 md:gap-5'>
                    ...
                    <div className='flex items-center gap-5 mt-2'>
                        <div className='flex flex-col'>
                            <p className='text-[#c5a059] text-lg md:text-2xl md:text-3xl'>{Object.keys(numbersOfPlaceByCountry).length}</p>
                            <p className='text-gray-400 uppercase text-xs'>Countries</p>
                        </div>
                        <PiLineVertical className='text-gray-400 text-3xl' />
                        <div className='flex flex-col'>
                            <p className='text-[#c5a059] text-lg md:text-2xl md:text-3xl'>{totalPlace}</p>
                            <p className='text-gray-400 uppercase text-xs'>Trips</p>
                        </div>
                        <PiLineVertical className='text-gray-400 text-3xl' />
                        <div className='flex flex-col'>
                            <p className='text-[#c5a059] text-lg md:text-2xl md:text-3xl'>{totalDays}</p>
                ...
                <div className='relative flex flex-col gap-4 md:gap-8 p-8 backdrop-blur rounded-2xl text-white border border-gray-600 w-[270px] sm:w-[300px] md:w-[350px] lg:w-[450px]'>
                    <h1 className='font-serif text-3xl'>{country}</h1>
                    <div className='flex items-center gap-2 text-xs text-gray-400'>
                        <FiCalendar />
                        <span>{formatDateStart} / {formatDateEnd}</span>
                    </div>
                    <img className='rounded-2xl object-cover aspect-[4/3]' src={image} alt={place} />
                    <p className='italic text-xs text-gray-400'>{place}</p>
